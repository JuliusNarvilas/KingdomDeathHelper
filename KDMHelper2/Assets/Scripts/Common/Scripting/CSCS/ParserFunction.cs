...
namespace SplitAndMerge
{
    public class ParserFunction
    {
        ...
        public ParserFunction(ParsingScript script, string item, char ch, ref string action)
        {
            ...
            if (m_impl == s_strOrNumFunction && string.IsNullOrEmpty(item))
            {
                string problem = (!string.IsNullOrEmpty(action) ? action : ch.ToString());
        ...
        public static ParserFunction GetFromNamespace(string name, string nameSpace, ParsingScript script)
        {
            ...
            if (!string.IsNullOrEmpty(prop) &&  impl is GetVarFunction)
            {
        ...
        public static void AddGlobalOrLocalVariable(string name, GetVarFunction function)
        {
            ...
            if (lastLevel != null && s_locals.Peek().IsNamespace && !string.IsNullOrEmpty(s_namespace))
            {
                                 ...
                                 StringBuilder sb, bool isLocal = false)
        {
            ...
            for (int i = 0; i < all.Count; i++)
            {
                ...
                if (gvf == null || string.IsNullOrEmpty(variable.Name))
                {
                    continue;
                }
