...
namespace SplitAndMerge
{
    public class ParsingScript
    {
        ...
        public string GetStack(int firstOffset = 0)
        {
            ...
            while (script != null)
            {
                ...
                string filename = string.IsNullOrEmpty(script.Filename) ? "" :
                                  Utils.GetFullPath(script.Filename);
                string line = string.IsNullOrEmpty(filename) || !File.Exists(filename) ? "" :
                              File.ReadAllLines(filename).Skip(lineNumber).Take(1).First();
    ...
    {
        private string m_exceptionStack = string.Empty;
